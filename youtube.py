...
image = Image.open(r"C:\Users\Barry\Desktop\projects\youtube_warehouse\image.jpg")
...
api_key = 'AIzaSyA12xodukoLjFdsqRqbqKeBJuBb8pWF1iY'  #get your api_key which is saved in os.environ
...
def migrate_to_mysql(name):
    ...
    #Table video
    table_video = {'video_id': [], 'playlist_id': [], 'video_name': [], 'video_description': [], 'published_date': [],
                   'view_count': [],
    ...
    table_playlist_1.to_sql(name='table_playlist', con=engine, if_exists='replace', index=False)
    table_channel_1.to_sql(name='table_channel', con=engine, if_exists='replace', index=False)
    table_comment_1.to_sql(name='table_comment', con=engine, if_exists='replace', index=False)
    table_video_1.to_sql(name='table_video', con=engine, if_exists='replace', index=False)
...
if add_selectbox == "Scrape data":
    ...
    if st.button('analyse the data'):
        ...
        # Connect to the database
        cnx = mysql.connector.connect(**config)
        query = "SELECT video_name,view_count FROM table_video order by view_count desc limit 10;"
